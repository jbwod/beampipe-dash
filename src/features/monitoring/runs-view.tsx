...
export function RunsView() {
  ...
  const query = new URLSearchParams({ items_per_page: "200" });
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <div className="mb-4 grid gap-2 border border-[var(--bp-border)] bg-[var(--bp-panel)] p-2 sm:grid-cols-[minmax(180px,1fr)_180px_180px_auto]">
        ...
        <label><span className="sr-only">Status</span><select className="h-9 w-full border border-[var(--bp-border-soft)] bg-black px-2 text-xs" onChange={(event) => setStatus(event.target.value)} value={status}>{statuses.map((item) => <option key={item || "all"} value={item}>{item ? item.replaceAll("_", " ") : "all statuses"}</option>)}</select></label>
