  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <section className="mb-4 grid grid-cols-2 border-t border-l border-[var(--bp-border)] md:grid-cols-4">
        ...
        <MetricCell label="registered sources loaded" value={sources.data ? `${sources.data.length}${sources.hasNextPage ? "+" : ""}` : "--"} />
        ...
        {projects.isError ? <QueryFailure message="Project registry is unavailable" retry={() => projects.refetch()} /> : (
          <><TableFrame className="border-0"><DataTable className="min-w-[860px]"><TableHead><tr><Th>Project</Th><Th className="w-[100px]">Version</Th><Th className="w-[130px]">Contract</Th><Th className="w-[118px]">Sources loaded</Th><Th className="w-[118px]">Pending loaded</Th><Th className="w-[118px]">Runs</Th><Th className="w-[130px]">Latest run</Th><Th className="w-[112px]"><span className="sr-only">Action</span></Th></tr></TableHead><tbody>
            {projects.data?.map((project) => {
              ...
              return <tr className="hover:bg-[var(--bp-panel-soft)]" key={project.project_id}><Td><span className="inline-flex items-center gap-2"><Braces className="size-3.5 text-[var(--bp-cyan)]" /><span className="text-[var(--bp-highlight)]">{project.project_id}</span></span></Td><Td className="tabular-nums">v{project.version}</Td><Td><StatusBadge status={report?.valid === false ? "error" : report?.warnings?.length ? "warning" : "valid"} /></Td><Td className="tabular-nums">{projectSources.length}{sources.hasNextPage ? "+" : ""}</Td><Td className="tabular-nums text-[var(--bp-amber)]">{projectSources.filter((source) => source.workflow_run_pending).length}{sources.hasNextPage ? "+" : ""}</Td><Td className="tabular-nums">{projectRuns.length}{executions.data && executions.data.total > executions.data.items.length ? "+" : ""}</Td><Td className="text-[10px] text-[var(--bp-muted)]">{formatAge(projectRuns[0]?.created_at)}</Td><Td><Link className="text-[10px] uppercase text-[var(--bp-cyan)] hover:underline" href={`/projects/new?project=${encodeURIComponent(project.project_id)}`}>Open studio</Link></Td></tr>;
            })}
            {!projects.data?.length ? <tr><td colSpan={8}><EmptyRows message="no projects configured" /></td></tr> : null}
          </tbody></DataTable></TableFrame><PaginationControls hasNext={Boolean(sources.hasNextPage) && !sources.isFetchingNextPage} nextLabel={sources.isFetchingNextPage ? "Loading" : "Load more sources"} onNext={() => sources.fetchNextPage()} /></>
        )}
      </section>
    </div>
  );
