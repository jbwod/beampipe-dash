...
export function SourceExplorer({ id }: { id: string }) {
  ...
  const blockers = Array.isArray(status.data?.blockers) ? status.data.blockers : [];
  const metadataRows = Array.isArray(metadata.data?.metadata) ? metadata.data.metadata : [];
  ...
  return <div className="p-4 sm:p-6">
    ...
    <section className="mb-4 border border-[var(--bp-border)]"><SectionHeading detail={status.data?.ready_for_execution ? "all readiness gates pass" : `${blockers.length} blocker(s)`} title="Execution readiness" />{status.isPending ? <LoadingRows rows={2} /> : <div className="grid divide-y divide-[var(--bp-border-soft)] sm:grid-cols-4 sm:divide-x sm:divide-y-0"><Readiness label="Discovery" status={status.data?.discovery_complete ? "complete" : "incomplete"} /><Readiness label="Workflow" status={status.data?.workflow_run_pending ? "pending" : "settled"} /><Readiness label="Signature" status={status.data?.signature_matches_last_execution ? "already executed" : "new"} /><Readiness label="Admission" status={status.data?.ready_for_execution ? "ready" : "blocked"} /></div>}{blockers.length ? <div className="divide-y divide-[var(--bp-border-soft)] border-t border-[var(--bp-border)]">{blockers.map((blocker) => <p className="px-3 py-2 text-[10px] leading-5 text-[var(--bp-amber)]" key={blocker}>! {blocker}</p>)}</div> : null}</section>
