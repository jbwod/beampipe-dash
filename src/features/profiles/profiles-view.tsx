...
function SshRuntimePanel({ configured, diagnostics }: { configured: boolean | null; diagnostics: Array<{ message: string; severity: string }> }) {
  return <section className="border-t border-[var(--bp-border)]"><SectionHeading detail="external runtime credentials; no private key material is stored in this profile" title="SSH key runtime" /><div className="grid divide-y divide-[var(--bp-border-soft)] sm:grid-cols-3 sm:divide-x sm:divide-y-0"><RuntimeCheck icon={<KeyRound className="size-4" />} label="Private key" status={configured == null ? "checking" : configured ? "configured" : "not_configured"} value="file or environment" /><RuntimeCheck icon={<ShieldCheck className="size-4" />} label="Host verification" status={diagnostics.length ? diagnostics[0].severity : "profile_managed"} value={diagnostics[0]?.message ?? "validated on connection"} /><RuntimeCheck icon={<ServerCog className="size-4" />} label="Credential check" status={configured && !diagnostics.length ? "ready" : "pending"} value="beampipe security check" /></div></section>;
}
